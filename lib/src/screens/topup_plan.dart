...
import 'package:selfcare/util/DataPrice.dart';
import 'package:selfcare/util/line_dash_widget.dart';

class TopUpPlan extends StatefulWidget {
  ...
  double amount, dataVolume;
  var rand;

  TopUpPlan({Key key, LoginResponse loginResponse, int index})
  ...
  @override
  _TopUpPlanState createState() => _TopUpPlanState();
}

class _TopUpPlanState extends State<TopUpPlan>
  ...
  List<DropdownMenuItem<DataPrice>> _dropdownMenuItems;
  DataPrice _selectedData = DataPrice(dataVolume: 0.0, amount: 0.0);
  ...
  void initState() {
    ...
    widget.amount =
        widget._loginResponse.data.accounts[widget.index].product.price;
    widget.dataVolume = widget._loginResponse.data.accounts[widget.index]
        .product.display.dataCapacity.capacity;

    period = DataPrice.getPlan(widget.amount, widget.dataVolume);
    _dropdownMenuItems = buildDropdownMenuItems(period);
    _selectedData = _dropdownMenuItems[0].value;

    widget.vat = _selectedData.amount != null ? 0.075 * _selectedData.amount : 0;
    ...
    platform.setMethodCallHandler((call) {
      ...
      switch (call.method) {
        case "onSuccess":
          dataplanBloc
              .dispatch(ReferenceConfirmClickEvent(reference: argument));
      ...
      child: BlocListener<DataplanBloc, DataplanState>(
        listener: (context, state) async {
          ...
          if (state is TransactionSuccessFul) {
            String response = "";
            try {
              final String result = await platform.invokeMethod('chargeCard',
                  {"reference": state.transactionResponse.data.transRef});
          ...
          if (state is TransactionSuccessFully) {
            showDialog(
              ...
              builder: (BuildContext context) {
                ...
                return AlertDialog(
                  title: new Text("Data Topup Successful"),
                  content: new Text(
  ...
  Widget buildButtonWidget() {
    return InkWell(
      onTap: () async {
        if (widget._loginResponse.data.accounts[widget.index].product.display
            .canRenew) {
          ...
          var loading = FlushbarHelper.createLoading(
              message: 'Cannot perform top up on this account')
            ..show(context);
          Future.delayed(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
            ...
            gradient: LinearGradient(
                ...
                colors: [Color(0xFF4A81C3), Color(0xFF4A81C3)])),
        child: Text(
          'PAY   N${_selectedData.amount != null ? _selectedData.amount + widget.vat : 0}',
          style: TextStyle(
              fontSize: 18,
              fontFamily: "Cabin",
  ...
  List<DropdownMenuItem<DataPrice>> buildDropdownMenuItems(List dataVolume) {
    List<DropdownMenuItem<DataPrice>> items = List();
    for (DataPrice data in dataVolume) {

      items.add(
        DropdownMenuItem(
          value: data,
          child: Text("  Amount: N  ${data.amount.toStringAsFixed(1)}  - Data:  ${data.dataVolume.toStringAsFixed(1)}GB"),
        ),
      );
    }
    return items;
  ...
  Widget _buildPeriodChoiceWidget(BuildContext context, List data) {
    return Padding(
      ...
      child: Container(
        ...
        child: DropdownButton<DataPrice>(
          isExpanded: true,
          hint: Padding(
...
//                });
  ...
  _body(context) {
    widget.vat = 0.075 * _selectedData.amount;
          ...
          child: Column(
            ...
            children: <Widget>[
              Padding(
                ...
                child: Text(
                  "${widget._loginResponse.data.accounts[widget.index].portaoneAccountCredential.portaoneUsername}",
              ...
              Padding(
                ...
                child: Text(
                  "${widget._loginResponse.data.accounts[widget.index].product.name} - N${widget._loginResponse.data.accounts[widget.index].product.fundPrice.toStringAsFixed(2)}",
                  textAlign: TextAlign.start,
                  style: TextStyle(
                    fontSize: 14,
                    fontFamily: "Cabin",
              ...
              Padding(
                ...
                child: _buildPeriodChoiceWidget(context, period),
              ),
              ...
              Padding(
                ...
                child: Row(
                  children: <Widget>[
                    Text(
                      "${widget._loginResponse.data.accounts[widget.index].product.validity} Month",
                      textAlign: TextAlign.start,
                      style: TextStyle(
                        fontFamily: "Cabin",
                        fontSize: 14,
                    ...
                    Spacer(),
                    Text(
                      "${widget._loginResponse.data.accounts[widget.index].product.display.dataCapacity.capacity}GB",
                      //"${state.bookingForm.schedule.busName}",
                      textAlign: TextAlign.end,
                      style: TextStyle(
                        fontSize: 14,
...
//                  ),
  ...
  onChangedDropDownItem(DataPrice value) {
    setState(() {
      _selectedData = value;
    });
  }
